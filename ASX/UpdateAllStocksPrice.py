...
def populate_dates ():
    ...
    for i in range(len(stocks)):
        stock_csv = data_csv.loc[data_csv['ticker'] == stocks[i]]
        stock_csv.sort_values(by=['date'])
        dates.append(stock_csv.reset_index(drop=True)['date'][0])
        print(str(i+1) + "/" + str(len(stocks)))
    print(dates)
    print("Delaying for: " + str(60-(time.time() - timeStart)))
    time.sleep(60-(time.time() - timeStart))
...
def internet_resource_getter(index, prox, stock):
    ...
    response = session.get('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=ASX:'+ticker+'&apikey='+key, proxies = proxyDict)
...
if __name__ == '__main__':
    ...
    for i in range(math.floor(len(stocks)/5)):
        ...
        if not (pool_outputs[0].empty):
            current_output = pool_outputs[0].reset_index()
            current_output = current_output.loc[current_output['index'] > dates[globalIndex]]
            print(stocks[globalIndex] + " - " + str(len(current_output)) + " Rows Added")
            total_append = current_output
            for j in range(1, len(pool_outputs)):
                if not (pool_outputs[j].empty):
                    current_output = pool_outputs[j].reset_index()
                    current_output = current_output.loc[current_output['index'] > dates[globalIndex+j]]
                    print(stocks[globalIndex+j] + " - " + str(len(current_output)) + " Rows Added")
                    total_append = total_append.append(current_output)

            with open('data.csv', 'a') as fd:
                fd.write(total_append.to_csv(header=False, index=False))
            print("SAVED! - "+str((i+1)*5)+"/"+str(len(stocks))+" - DELAYING FOR " + str(60-(time.time()-timeBefore)))
        else:
    ...
    amountLeft = len(stocks)-math.floor(len(stocks)/5)
    print("FETCHING LAST SET")
    pool = multiprocessing.Pool(processes=4)
    pool_outputs = pool.map(partial(internet_resource_getter, prox=proxyIndex, stock=globalIndex), range(0, amountLeft))
    pool.close()
    pool.join()
    print("FETCHED")
    timeBefore = time.time()
    if not (pool_outputs[0].empty):
        print(pool_outputs[0].reset_index())
        current_output = pool_outputs[0].reset_index()
        total_append = current_output.loc[current_output['index'] > dates[globalIndex]]
        for j in range(1, len(pool_outputs)):
            if not (pool_outputs[j].empty):
                current_output = pool_outputs[j].reset_index()
                total_append = total_append.append(current_output.loc[current_output['index'] > dates[globalIndex + j]])
